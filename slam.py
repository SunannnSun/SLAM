...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y):
        ...
        gridCells = np.vstack((np.rint((x - s.xmin)/s.resolution), np.rint((y - s.ymin)/s.resolution))).astype(np.int32)
...
class slam_t:
    ...
    def rays2world(s, p, d, head_angle=0, neck_angle=0, angles=None):
        ...
        # First of all, recognizing we are ultimately only concerned with the (x, y) locations of the end point of each ray
        # as long as we have all the transformation at hand, we can perform simple matrix multiplication to get (x, y) in 
        # the world frame

        d = np.clip(d, s.lidar_dmin, s.lidar_dmax)
        numRays = angles.shape[0]
        xyzLidarArr = np.vstack((cos(angles), sin(angles), np.zeros((numRays, ))))
        xyzLidarArr = xyzLidarArr @ np.diag(d)

        lidar_to_body = euler_to_se3(0, head_angle, neck_angle, np.array([0, 0, s.lidar_height]))
        xyzBodyArr = lidar_to_body @ make_homogeneous_coords_3d(xyzLidarArr)

        body_to_world = euler_to_se3(0, 0, p[2], np.array([0, 0, s.head_height]))
        xyzWorldArr = body_to_world @ xyzBodyArr

        xyWorldArr = xyzWorldArr[0:2, :]
        ...
        # p is the pose of the particles expressed in the world frame, we can understand this as the origin of the body frame 
        # by lifting it to the top head position
    ...
    def get_control(s, t):
        ...
        xyth_t_1 = s.lidar[t-1]['xyth']
        xyth_t   = s.lidar[t]['xyth']

        controlInput = smart_minus_2d(xyth_t, xyth_t_1)
    ...
    def dynamics_step(s, t):
        ...
        controlInput = s.get_control(t)

        particles = s.p
        new_particles = np.zeros(particles.shape)
        noise = np.random.multivariate_normal(mean = np.zeros((3, )), cov = s.Q, size = particles.shape[1])
        for i in range(particles.shape[1]):
            new_particles[:, i] = smart_plus_2d(smart_plus_2d(particles[:, i], controlInput), noise[i, :])
    ...
    def update_weights(w, obs_logp):
        ...
        log_old_weights = np.log(w) + obs_logp
        logSumExp = slam_t.log_sum_exp(log_old_weights)
        new_weights = np.exp(log_old_weights - logSumExp)
    ...
    def observation_step(s, t):
        ...
        """
        particles       = s.p
        lidarScanOutput = s.lidar[t]['scan']
        time            = s.lidar[t]['t']
        t_joint         = s.find_joint_t_idx_from_lidar(time)
        headAngle       = s.joint['head_angles'][0, t_joint]
        neckAngle       = s.joint['head_angles'][1, t_joint]
        ...
        if t!= 0:
            ...
            for i in range(s.n):
                obs_i = s.rays2world(particles[:, i], lidarScanOutput, headAngle, neckAngle, s.lidar_angles)
                obs_i_cell_idx =  s.map.grid_cell_from_xy(obs_i[0, :], obs_i[1, :])                
                obs_logp[i] = np.sum(s.map.cells[obs_i_cell_idx[0, :], obs_i_cell_idx[1, :]].reshape(-1, 1))
            

            s.w = s.update_weights(s.w, obs_logp)
            s.resample_particles()

        _, unique_idx, unique_count = np.unique(s.p, return_index=True, return_counts=True, axis=1)
        largest_weight_idx = unique_idx[np.argmax(unique_count)]


        obs = s.rays2world(particles[:, largest_weight_idx], lidarScanOutput, headAngle, neckAngle, s.lidar_angles)
        obs_cell_idx = s.map.grid_cell_from_xy(obs[0, :], obs[1, :])
        ...
        rows = obs_cell_idx[0, :]
        cols = obs_cell_idx[1, :]
        s.map.log_odds[rows, cols] += s.lidar_log_odds_occ

        for i in range(obs_cell_idx.shape[1]):
            s.map.log_odds[obs_cell_idx[0, i], obs_cell_idx[1, i]] -= s.lidar_log_odds_free
            s.map.log_odds[obs_cell_idx[0, i], obs_cell_idx[1, i]] += s.lidar_log_odds_occ
